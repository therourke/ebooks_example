  ...
  def configure
    ...
    if can_run?
      ...
      if initial_corpus_file.blank? || in_cloud?(File.basename(@archive_path))
        ...
        @archive = Boodoo.make_Archive(original, path: archive_path, client: twitter, local:false)
        ...
        @archive = Boodoo.make_Archive(original, path: archive_path, client: twitter, local: false, content: archive_json.lines)
